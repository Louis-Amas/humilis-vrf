...
library VRF {
  ...
  // Order of the curve
  uint public constant NN = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141;
  ...
  function derivePoint(uint _d, uint _x, uint _y) internal pure returns (uint, uint) {
  ...
  function deriveY(uint8 _yByte, uint _x) internal pure returns (uint) {
  ...
  function gammaToHash(uint _gammaX, uint _gammaY) internal pure returns (bytes32) {
  ...
  function verify(uint[2] memory _publicKey, uint[4] memory _proof, bytes memory _message) internal pure returns (bool) {
    // Step 2: Hash to try and increment (outputs a hashed value, a finite EC point in G)
    (uint hPointX, uint hPointY) = hashToTryAndIncrement(_publicKey, _message);

    // Step 3: U = s*B - c*Y (where B is the generator)
    (uint uPointX, uint uPointY) = ecMulSubMul(_proof[3], GX, GY, _proof[2], _publicKey[0], _publicKey[1]);

    // Step 4: V = s*H - c*Gamma
    (uint vPointX, uint vPointY) = ecMulSubMul(_proof[3], hPointX, hPointY, _proof[2], _proof[0], _proof[1]);
  ...
  /// @return true, if VRF proof is valid
  function fastVerify(
    uint[2] memory _publicKey,
    uint[4] memory _proof,
    bytes memory _message,
    uint[2] memory _uPoint,
    uint[4] memory _vComponents
    ...
    (uint vPointX, uint vPointY) =
      EllipticCurve.ecSub(_vComponents[0], _vComponents[1], _vComponents[2], _vComponents[3], AA, PP);
  ...
  function decodeProof(bytes memory _proof) internal pure returns (uint[4] memory) {
    ...
    uint gammaY = deriveY(gammaSign, gammaX);
  ...
  function computeFastVerifyParams(uint[2] memory _publicKey, uint[4] memory _proof, bytes memory _message)
    internal
    pure
    returns (uint[2] memory, uint[4] memory)
  {
    // Requirements for Step 3: U = s*B - c*Y (where B is the generator)
    (uint hPointX, uint hPointY) = hashToTryAndIncrement(_publicKey, _message);
    (uint uPointX, uint uPointY) = ecMulSubMul(_proof[3], GX, GY, _proof[2], _publicKey[0], _publicKey[1]);
    // Requirements for Step 4: V = s*H - c*Gamma
    (uint sHX, uint sHY) = derivePoint(_proof[3], hPointX, hPointY);
    (uint cGammaX, uint cGammaY) = derivePoint(_proof[2], _proof[0], _proof[1]);
  ...
  function hashToTryAndIncrement(uint[2] memory _publicKey, bytes memory _message) internal pure returns (uint, uint) {
  ...
  function encodePoint(uint _x, uint _y) internal pure returns (bytes memory) {
  ...
  function ecMulSubMul(uint _scalar1, uint _a1, uint _a2, uint _scalar2, uint _b1, uint _b2)
    internal
    pure
    returns (uint, uint)
  {
    (uint m1, uint m2) = derivePoint(_scalar1, _a1, _a2);
    (uint n1, uint n2) = derivePoint(_scalar2, _b1, _b2);
    (uint r1, uint r2) = EllipticCurve.ecSub(m1, m2, n1, n2, AA, PP);
  ...
  function ecMulVerify(uint _scalar, uint _x, uint _y, uint _qx, uint _qy) internal pure returns (bool) {
  ...
  /// @return true, if first 20 bytes match
  function ecMulSubMulVerify(uint _scalar1, uint _scalar2, uint _x, uint _y, uint _qx, uint _qy)
    internal
    pure
    returns (bool)
  {
    uint scalar1 = (NN - _scalar1) % NN;
    scalar1 = mulmod(scalar1, _x, NN);
    uint scalar2 = (NN - _scalar2) % NN;
  ...
  function pointToAddress(uint _x, uint _y) internal pure returns (address) {
    return address(uint160(uint(keccak256(abi.encodePacked(_x, _y)))) & 0x00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF);
  }
}
