...
library EllipticCurve {
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        // s
        uint256 s = mulmod(4, mulmod(_x, square[1], _pp), _pp);
        // m
        uint256 m = addmod(mulmod(3, square[0], _pp), mulmod(_aa, mulmod(square[2], square[2], _pp), _pp), _pp);
        // qx
        uint256 qx = addmod(mulmod(m, m, _pp), _pp - addmod(s, s, _pp), _pp);
        // qy = -8*y1^4 + M(S-T)
        uint256 qy = addmod(
            mulmod(m, addmod(s, _pp - qx, _pp), _pp), _pp - mulmod(8, mulmod(square[1], square[1], _pp), _pp), _pp
        );
        // qz = 2*y1*z1
